...
func RunBenchmark(ctx context.Context, opts RunOptions) error {
	...
	result := Result{
		...
		AdvertisedSpecs: map[string]string{
			"vcpu":      spec.AdvertisedVCPU,
			"ram":       spec.AdvertisedRAM,
			"disk":      spec.AdvertisedDisk,
			"tier_name": spec.TierName,
		...
		BenchConfig: BenchConfigInfo{
			...
			MixedDurationSec:    cfg.MixedDurationSec,
	...
	if !opts.SkipLoad {
		...
		fmt.Printf(
			"  loaded in %.2fs (%.1f nodes/s, %.1f edges/s)\n",
			lr.WallClockSeconds,
			lr.NodesPerSec(),
			lr.EdgesPerSec(),
		)
		...
		result.Ingest = &IngestInfo{
			NodesLoaded:      lr.NodesLoaded,
			EdgesLoaded:      lr.EdgesLoaded,
			WallClockSeconds: round3(lr.WallClockSeconds),
			NodesPerSec:      round2(lr.NodesPerSec()),
			EdgesPerSec:      round2(lr.EdgesPerSec()),
		}
	} else {
	...
	for _, hops := range []int{1, 2, 3} {
		summary, coldMs, err := runTraversalWorkload(
			ctx,
			client,
			nodeIDs,
			hops,
			cfg.TraversalSampleSize,
			cfg.Warmup,
			rng,
		)
		...
		if err != nil {
			result.Caveats = append(
				result.Caveats,
				fmt.Sprintf("%d-hop traversal failed: %v", hops, err),
			)

			result.Traversals[fmt.Sprintf("%d_hop", hops)] = HopResult{
				Summary:     LatencySummary{},
				ColdStartMs: 0,
			}

			fmt.Printf("  %d-hop: FAILED (%v)\n", hops, err)
			continue
		}

		result.Traversals[fmt.Sprintf("%d_hop", hops)] = HopResult{
			Summary:     summary,
			ColdStartMs: round3(coldMs),
		}

		fmt.Printf(
			"  %d-hop: p50=%.2fms  p95=%.2fms  cold=%.2fms\n",
			hops,
			deref(summary.P50Ms),
			deref(summary.P95Ms),
			coldMs,
	...
	pointSummary, pointCold, err := runReadWorkload(
		nodeIDs,
		cfg.Iterations,
		cfg.Warmup,
		rng,
	...
	result.Lookups["point_lookup"] = LookupResult{
		Summary:     pointSummary,
		ColdStartMs: round3(pointCold),
	}

	fmt.Printf(
		"  point lookup: p50=%.2fms  p95=%.2fms\n",
		deref(pointSummary.P50Ms),
		deref(pointSummary.P95Ms),
	)

	idxSummary, idxCold, err := runReadWorkload(
		nodeIDs,
		cfg.Iterations,
		cfg.Warmup,
		rng,
	...
	result.Lookups["indexed_range_lookup"] = LookupResult{
		Summary:     idxSummary,
		ColdStartMs: round3(idxCold),
	}

	fmt.Printf(
		"  indexed range lookup: p50=%.2fms  p95=%.2fms\n",
		deref(idxSummary.P50Ms),
		deref(idxSummary.P95Ms),
	)
	...
	var aggSummary LatencySummary

	for i := 0; i < cfg.Warmup; i++ {
		_, err := client.AggregationTopOutDegree(ctx, 10)
		if err != nil {
			aggregationFailed = true

			result.Caveats = append(
				result.Caveats,
				fmt.Sprintf("aggregation warmup failed: %v", err),
			)

			fmt.Printf(
				"  aggregation: FAILED during warmup (%v)\n",
				err,
			)
	...
	if !aggregationFailed {
		...
		for i := 0; i < cfg.Iterations; i++ {
			...
			if err != nil {
				...
				result.Caveats = append(
					result.Caveats,
					fmt.Sprintf("aggregation failed after %d/%d iterations: %v", i, cfg.Iterations, err),
				)

				fmt.Printf(
					"  aggregation: FAILED after %d/%d iterations (%v)\n",
					i,
					cfg.Iterations,
					err,
				)
		...
		if !aggregationFailed && len(aggLatencies) > 0 {
			aggSummary = SummarizeLatencies(aggLatencies)

			result.Aggregation["top10_out_degree"] = aggSummary

			fmt.Printf(
				"  aggregation: p50=%.2fms  p95=%.2fms\n",
				deref(aggSummary.P50Ms),
				deref(aggSummary.P95Ms),
			)
		}
	}
	...
	fmt.Printf(
		"running mixed workload sweep (concurrency=%v, %d%% reads, %ds per step)...\n",
		cfg.ConcurrencyLevels,
		int(cfg.ReadWriteRatio*100),
		cfg.MixedDurationSec,
	)

	mixed, err := runMixedWorkloadSweep(
		ctx,
		opts.PlatformKey,
		opts.Env,
		nodeIDs,
	...
	// IMPORTANT:
	// Always preserve successful concurrency levels that completed before
	// a later concurrency level failed.
	result.MixedWorkload = mixed

	if err != nil {
		result.Caveats = append(
			result.Caveats,
			fmt.Sprintf("mixed workload partially failed: %v", err),
		)

		fmt.Printf(
			"  mixed workload partially failed: %v\n",
			err,
		)
	}
	...
	result.Footprint = FootprintInfo{
		Observable:  fp.Observable,
		StoredBytes: fp.StoredBytes,
		Notes:       fp.Notes,
	}
	...
	outPath := filepath.Join(
		outDir,
		fmt.Sprintf("%s_%s.json", opts.PlatformKey, ts),
	)

	latestPath := filepath.Join(
		outDir,
		fmt.Sprintf("%s_latest.json", opts.PlatformKey),
	)
	...
	fmt.Printf("\nwrote %s\n", outPath)
	fmt.Printf(
		"wrote %s  (build-report reads the *_latest.json files)\n",
		latestPath,
	)
...
func runReadWorkload(
	sampleIDs []int64,
	iterations,
	warmup int,
	rng *rand.Rand,
	fn func(id int64) error,
) (LatencySummary, float64, error) {
...
func runTraversalWorkload(
	ctx context.Context,
	client dbclient.GraphClient,
	nodeIDs []int64,
	hops,
	sampleSize,
	warmup int,
...
func runMixedWorkloadSweep(
	...
	for _, concurrency := range concurrencyLevels {
		...
		for o := range ch {
			if o.err != nil {
				// Preserve every successful concurrency level that was
				// already stored in "out". The caller will record this
				// concurrency as a caveat instead of inventing metrics.
				fmt.Printf(
					"    concurrency=%3d -> FAILED (%v)\n",
					concurrency,
					o.err,
				)
		...
		throughput := round2(
			float64(totalOps) / float64(durationSec),
		)
		...
		fmt.Printf(
			"    concurrency=%3d  ->  %8.2f qps (p50=%.2fms, p95=%.2fms)\n",
			concurrency,
			throughput,
			deref(summary.P50Ms),
			deref(summary.P95Ms),
